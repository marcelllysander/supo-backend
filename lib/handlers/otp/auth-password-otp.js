...
async function handleSignupRequest(req, res, body) {
  ...
  // kalau nomor HP berubah, simpan perubahan dan reset state OTP phone
  if (phoneChanged) {
  ...
  // penting: update data dasar dulu, walaupun email sudah verified
  await ref.set(baseData, { merge: true });

  const freshSnap = await ref.get();

  if (purpose === "email" && freshSnap.exists && freshSnap.get("emailVerifiedAt")) {
    return res.status(200).json({
      ok: true,
  ...
  if (purpose === "phone" && freshSnap.exists && freshSnap.get("phoneVerifiedAt") && !phoneChanged) {
    return res.status(200).json({
      ok: true,
