...
function getFlow(body) {
  ...
  // fallback sederhana agar request signup yang lupa kirim flow
  // tetap bisa terbaca sebagai signup
...
async function handleSignupRequest(req, res, body) {
  const purpose = String(body.purpose || "")
    .trim()
    .toLowerCase(); // email | phone
  const channel = String(body.channel || "")
    .trim()
    .toLowerCase(); // whatsapp | sms
  ...
  // Untuk purpose=phone, baru wajib ada phone valid
  if (purpose === "phone") {
    if (!phoneE164) {
      return res.status(400).json({
        ok: false,
  ...
  const phoneChanged = prevPhone && phoneE164 && prevPhone !== phoneE164;
...
async function handleSignupVerify(req, res, body) {
  ...
  // Untuk verify phone baru divalidasi
  if (purpose === "phone") {
...
async function handleSignupComplete(req, res, body) {
  ...
  const phoneE164 = normalizePhoneE164(String(snap.get("phoneE164") || ""));
  ...
  // Phone tidak wajib, jadi cek registry hanya jika phone memang ada
  if (phoneE164) {
  ...
  try {
    ...
    const userData = {
      ...
      // phone boleh kosong karena belum wajib
      phone: phoneE164 || "",
      phoneE164: phoneE164 || "",
      phoneVerified: false,
      phoneVerifiedVia: "",
    ...
    // simpan registry nomor HP hanya jika memang ada
    if (phoneE164) {
      const phoneRegistryRef = getPhoneRegistryRef(phoneE164);
      batch.set(
        phoneRegistryRef,
